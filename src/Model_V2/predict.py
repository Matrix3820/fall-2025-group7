...
class ModelPredictor:
    ...
    def create_prediction_visualization(self, results_df, save_path=None):
        ...
        base_path = predictions_dir / f"test_prediction_analysis_{model_version}" if save_path is None else Path(save_path).with_suffix("")
        ...
        if "td_or_asd" in results_df.columns:
            ...
            axes[1, 1].text(0.5, 0.5, f"Accuracy: {accuracy:.4f}", ha="center", va="center", transform=axes[1, 1].transAxes, fontsize=16)
...
def predict_from_file(data_path):
    ...
    if "td_or_asd" in results_df.columns:
        accuracy = accuracy_score(results_df["td_or_asd"], results_df["predicted_td_or_asd"])
        class_report = classification_report(results_df["td_or_asd"], results_df["predicted_td_or_asd"], output_dict=True)
        cm = confusion_matrix(results_df["td_or_asd"], results_df["predicted_td_or_asd"]).tolist()

...
def save_figure_multi_formats(fig, base_path, formats=("png", "pdf", "svg")):
    ...
    for ext in formats:
        fig.savefig(base_path.with_suffix(f".{ext}"), bbox_inches="tight", dpi=300)
...
class ModelVisualizer:
    def __init__(self):
        ...
        # Example: Results/V2/visualizations/
    ...
    def create_feature_importance_plot(self, explainability_data):
        ...
        plt.title(f"Top 20 Features – XGBoost {model_version}")
    ...
    def create_characteristic_importance_plot(self, explainability_data):
        ...
        ax1.set_title(f"Characteristic Importance – Model {model_version}")
        ...
        ax2.set_title("Feature Count per Characteristic")
    ...
    def create_td_vs_asd_comparison_plot(self, explainability_data):
        ...
        # Characteristic-based mention rates
        for char in characteristics:
            clean = char.replace(" ", "_").replace(",", "").replace("(", "").replace(")", "")
            td_vals.append(td_patterns[char].get(f"{clean}_mentioned", 0))
            asd_vals.append(asd_patterns[char].get(f"{clean}_mentioned", 0))

        # Add FSR / PE from training data
        train_data_path = (
        ...
        if train_data_path.exists():
            ...
            td_vals.append((td_fsr - fsr_min) / (fsr_max - fsr_min) if fsr_max > fsr_min else 0)
            asd_vals.append((asd_fsr - fsr_min) / (fsr_max - fsr_min) if fsr_max > fsr_min else 0)

            td_vals.append((td_pe - pe_min) / (pe_max - pe_min) if pe_max > pe_min else 0)
            asd_vals.append((asd_pe - pe_min) / (pe_max - pe_min) if pe_max > pe_min else 0)
        ...
        bars1 = ax.bar(x - width / 2, td_vals, width, label="TD", color="lightblue", alpha=0.7)
        bars2 = ax.bar(x + width / 2, asd_vals, width, label="ASD", color="lightcoral", alpha=0.7)

        ax.set_title(f"TD vs ASD – Characteristic Mention Patterns ({model_version})")
        ax.set_ylabel("Mention Rate (normalized)")
        ax.set_xticks(x)
    ...
    def create_feature_importance_by_target_plot(self, explainability_data):
        ...
        bars = ax1.bar(x, importances, color="steelblue", alpha=0.8)
        ax1.set_title(f"Top Features – Importance ({model_version})")
        ax1.set_xticks(x)
        ax1.set_xticklabels(feature_names, rotation=45, ha="right")

        # TD vs ASD comparisons
        ax2.bar(x - width / 2, td_vals, width, label="TD", color="lightblue", alpha=0.7)
        ax2.bar(x + width / 2, asd_vals, width, label="ASD", color="lightcoral", alpha=0.7)
        ax2.set_title("Feature Values by Group (TD vs ASD)")
        ax2.set_xticks(x)
    ...
    def create_model_performance_plot(self, training_results):
        ...
        ax1.bar(range(len(cv_scores)), cv_scores, color="green", alpha=0.7)
        ax1.set_title(f"Cross-Validation Scores – {model_version}")

        ax2.bar(["Mean CV Accuracy"], [mean_score], yerr=[std_score], capsize=10, color="blue", alpha=0.7)
    ...
    def create_confusion_matrix_plot(self):
        ...
        if not test_results_path.exists():
            print("No test results found. Skipping confusion matrix.")
            return
        ...
        plt.title(f"Confusion Matrix – {model_version}")
...
def create_visualizations():
    ...
    explainability_data, training_results = viz.load_results()

    viz.create_feature_importance_plot(explainability_data)
    viz.create_characteristic_importance_plot(explainability_data)
    viz.create_td_vs_asd_comparison_plot(explainability_data)
    viz.create_feature_importance_by_target_plot(explainability_data)
    viz.create_model_performance_plot(training_results)
    viz.create_confusion_matrix_plot()

    print(f"All visualizations saved to: {viz.viz_dir}")

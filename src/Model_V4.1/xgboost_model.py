...
class XGBoostClassifier:
    ...
    def train_model(self, X, y, groups=None, random_state=42):
        """
        Train XGBoost model with group-aware cross-validation (GroupKFold)
        to prevent participant-level leakage.
        """
        # Scale features
        ...
        # Convert scaled X back to DataFrame (for index alignment)
        X_scaled_df = pd.DataFrame(X_scaled, columns=self.feature_names, index=X.index)

        # Manual CV loop
        for fold, (train_idx, val_idx) in enumerate(gkf.split(X_scaled_df, y, groups=groups)):
...
def train_xgboost_model():
    ...
    groups = df['sub'] if 'sub' in df.columns else None
    print(f"Feature matrix shape: {X.shape}")
    print(f"Target distribution: {y.value_counts().to_dict()}")
    
    training_results = classifier.train_model(X, y,groups=groups)

...
IMPORTANT: Always return valid JSON format."""
        ...
        self.agent.set_parameters(max_tokens=1000, temperature=0.0)
        self.call_timeout_sec = 15 #-----
...
def process_features(df):
    ...
    processed_df['TDNorm_avg_PE_scaled'] = StandardScaler().fit_transform(processed_df[['TDNorm_avg_PE']])
    processed_df['overall_avg_PE_scaled'] = StandardScaler().fit_transform(processed_df[['overall_avg_PE']])
    ...
    for idx, row in tqdm(processed_df.iterrows(), total=len(processed_df), desc="Characteristic features"):
        char_features = char_extractor.extract_characteristic_features(row['free_response_TDprof_norm'], idx)

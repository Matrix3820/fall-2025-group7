...
def preprocess_training_data():
    ...
    trial_subcat_df, trial_cat_df = preprocess_trial_data()

    print("Merging Trial Level data with Base Dataset:")
    merged_df = pd.merge(df, trial_subcat_df, on=["sub", "profile"], how="inner")
    merged_df = pd.merge(merged_df, trial_cat_df, on=["sub", "profile"], how="inner")

    artifacts_dir = project_root / "Results" / model_version / "preprocessing"
    artifacts_dir.mkdir(parents=True, exist_ok=True)

    selected_columns = ['sub', 'profile', 'FSR', 'avg_PE', 'free_response', 'td_or_asd', 'mean_slope_cat',
                        'mean_slope_overall_cat','mean_slope_subcat', 'mean_slope_overall_subcat']
    df_filtered = merged_df[selected_columns].copy()

    df_filtered = df_filtered.dropna(subset=['FSR', 'avg_PE', 'free_response','mean_slope_cat',
                        'mean_slope_overall_cat','mean_slope_subcat', 'mean_slope_overall_subcat'])

...
model_v1_dir = current_dir.parent / "Model_V1"

# Add the Model_V1 directory to the path
if str(model_v1_dir) not in sys.path:
    sys.path.insert(0, str(model_v1_dir))
...
try:
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location("predict", model_v1_dir / "predict.py")
        ...
        st.error(f"Model_V1 directory path: {model_v1_dir}")
        st.error(f"Model_V1 exists: {model_v1_dir.exists()}")
        st.error(f"Current sys.path: {sys.path[:3]}")
        st.error(f"Files in Model_V1: {list(model_v1_dir.glob('*.py'))}")
        st.stop()
...
class DemoApp:
    ...
    def load_results(self):
        try:
            ...
            test_results_path = self.results_dir / 'comprehensive_test_results_v1.json'
    ...
    def show_results_page(self):
        ...
        with tab4:
            ...
            viz_files = [
                ('feature_importance_v1.png', 'Feature Importance'),
                ('characteristic_importance_v1.png', 'Characteristic Importance'),
                ('td_vs_asd_comparison_v1.png', 'TD vs ASD Comparison'),
                ('model_performance_v1.png', 'Model Performance'),
                ('confusion_matrix_v1.png', 'Confusion Matrix'),
                ('characteristic_ranking_v1.png', 'Characteristic Ranking')
            ]
        ...
        with tab5:
            ...
            test_pred_path = predictions_dir / 'test_prediction_analysis_v1.png'
            ...
            sample_contrib_path = predictions_dir / 'sample_feature_contributions_v1.png'
            ...
            test_predictions_path = predictions_dir / 'test_predictions_v1.csv'
            if test_predictions_path.exists():
                ...
                try:
                    ...
                    st.download_button(
                        ...
                        data=csv,
                        file_name="test_predictions_v1.csv",
                        mime="text/csv"
                    )
                            ...
                            with st.spinner("Processing batch predictions..."):
                                try:
                                    ...
                                    st.download_button(
                                        ...
                                        data=csv,
                                        file_name="batch_predictions_v1.csv",
                                        mime="text/csv"
                                    )

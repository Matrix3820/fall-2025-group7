...
class XGBoostClassifier:
    ...
    def load_data(self, data_path=None):
        if data_path is None:
            ...
            data_path = project_root / "data" / "Data_v2q" / "LLM_data_train_preprocessed_v2q.csv"
    ...
    def save_model(self, model_dir=None):
        if model_dir is None:
            current_dir = Path(__file__).parent
            project_root = current_dir.parent.parent
            model_dir = project_root / "Results" / "V2Q"
        
        model_dir = Path(model_dir)
        model_dir.mkdir(parents=True, exist_ok=True)
        
        with open(model_dir / 'xgboost_model_v2q.pkl', 'wb') as f:
            pickle.dump(self.model, f)
        
        with open(model_dir / 'scaler_v2q.pkl', 'wb') as f:
            pickle.dump(self.scaler, f)
        
        with open(model_dir / 'feature_names_v2q.pkl', 'wb') as f:
            pickle.dump(self.feature_names, f)
        
        if self.feature_importance:
            feature_importance_serializable = convert_numpy_types(self.feature_importance)
            with open(model_dir / 'feature_importance_v2q.json', 'w') as f:
    ...
    def load_model(self, model_dir=None):
        ...
        with open(model_dir / 'xgboost_model_v2q.pkl', 'rb') as f:
            self.model = pickle.load(f)
        
        with open(model_dir / 'scaler_v2q.pkl', 'rb') as f:
            self.scaler = pickle.load(f)
        
        with open(model_dir / 'feature_names_v2q.pkl', 'rb') as f:
            self.feature_names = pickle.load(f)
        
        feature_importance_path = model_dir / 'feature_importance_v2q.json'
...
def train_xgboost_model():
    ...
    current_dir = Path(__file__).parent
    project_root = current_dir.parent.parent
    results_dir = project_root / "Results" / "V2Q"
    
    training_results_serializable = convert_numpy_types(training_results)
    with open(results_dir / 'training_results_v2q.json', 'w') as f:

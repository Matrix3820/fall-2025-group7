...
class ModelPredictor:
    ...
    def create_prediction_visualization(self, results_df, save_path=None):
        if save_path is None:
            save_path = self.results_dir / "predictions" / "test_prediction_analysis_v2q.png"
    ...
    def save_predictions(self, results_df, filename='test_predictions_v2q.csv'):
...
def predict_from_file(data_path):
    ...
    if 'td_or_asd' in results_df.columns:
        ...
        results_path = predictor.results_dir / 'test_results_v2q.json'
...
def main():
    current_dir = Path(__file__).parent
    project_root = current_dir.parent.parent
    test_data_path = project_root / "data" / "Data_v2q" / "LLM_data_test_v2q.csv"
